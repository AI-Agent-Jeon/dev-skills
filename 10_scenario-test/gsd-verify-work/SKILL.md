...
```bash
INIT=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query init.verify-work "${PHASE_ARG}")
...
MVP_MODE=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query phase.mvp-mode "${phase_number}" --pick active)
```
</step>
...
```
UI_PHASE_FLAG=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
PHASE_GOAL=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query roadmap.get-phase "${phase_number}" --pick goal)
USER_STORY_VALID=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query user-story.validate --story "$PHASE_GOAL" --pick valid)
...
```bash
CHECKPOINT=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query uat.render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
`audit-open` is CJS-only until registered on `~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query`:

```bash
~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query audit-open --json
```
