...
```bash
CONFIG=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query state.load)
```
...
```bash
REVIEW_CMD=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query state.load 2>/dev/null | head -20)
   ```
...
```bash
~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query state.update "Last Activity" "$(date +%Y-%m-%d)"
~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
