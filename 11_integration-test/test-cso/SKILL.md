...
~/.claude/skills/dev-skills/bin/gstack/gstack-timeline-log '{"skill":"cso","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/dev-skills/bin/gstack/gstack-question-log '{"skill":"cso","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /test-cso — Chief Security Officer Audit (v2)
...
## User-invocable
When the user types `/test-cso`, run this skill.

## Arguments
- `/test-cso` — full daily audit (all phases, 8/10 confidence gate)
- `/test-cso --comprehensive` — monthly deep scan (all phases, 2/10 bar — surfaces more)
- `/test-cso --infra` — infrastructure-only (Phases 0-6, 12-14)
- `/test-cso --code` — code-only (Phases 0-1, 7, 9-11, 12-14)
- `/test-cso --skills` — skill supply chain only (Phases 0, 8, 12-14)
- `/test-cso --diff` — branch changes only (combinable with any above)
- `/test-cso --supply-chain` — dependency audit only (Phases 0, 3, 12-14)
- `/test-cso --owasp` — OWASP Top 10 only (Phases 0, 9, 12-14)
- `/test-cso --scope auth` — focused audit on a specific domain
...
**Daily mode (default, `/test-cso`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
**Comprehensive mode (`/test-cso --comprehensive`):** 2/10 confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include anything that MIGHT be a real issue. Flag these as `TENTATIVE` to distinguish from confirmed findings.
...
```bash
~/.claude/skills/dev-skills/bin/gstack/gstack-learnings-log '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**This tool is not a substitute for a professional security audit.** /test-cso is an AI-assisted
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use /test-cso as
...
**Always include this disclaimer at the end of every /test-cso report output.**
