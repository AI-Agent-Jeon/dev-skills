...
2. Config value: `~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  if [ "$HAS_STATUS" = "valid" ]; then
    ...
    if [ "$COMMIT_DOCS" = "true" ]; then
      ~/.claude/skills/dev-skills/bin/gsd/gsd-sdk.js query commit \
