...
~/.claude/skills/dev-skills/bin/gstack/gstack-timeline-log '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/dev-skills/bin/gstack/gstack-question-log '{"skill":"plan-ceo-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```
If a design doc exists (from `/init-office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.
...
from a prior CEO review session that paused so the user could run `/init-office-hours`. Use it
...
sure," or is clearly exploring rather than reviewing — offer `/init-office-hours`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /init-office-hours is designed for. Want to run /init-office-hours right now?
> We'll pick up right where we left off."

Options: A) Yes, run /init-office-hours now. B) No, keep going.
...
```bash
eval "$(~/.claude/skills/dev-skills/bin/gstack/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG/ceo-plans
```
...
**Persist the result:**
```bash
~/.claude/skills/dev-skills/bin/gstack/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
command breaks the review readiness dashboard in /deploy-ship.

```bash
~/.claude/skills/dev-skills/bin/gstack/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/dev-skills/bin/gstack/gstack-learnings-log '{"skill":"plan-ceo-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
