...
Tip: For scripted imports, use --domain <domain> to scope cookies to a single domain.`;
    ...
    case "download": {
      ...
      if (url.startsWith("blob:")) {
        ...
        buffer = fs5.readFileSync(tempPath);
        ...
        if (outputPath || isBase64) {
          ...
          fs5.renameSync(tempPath, finalPath);
...
function readPidCmdline(pid) {
  try {
    return fs16.readFileSync(`/proc/${pid}/cmdline`, "utf-8").replace(/\0/g, " ").trim();
  } catch {
    return "";
  }
}
...
// browse/src/browser-manager.ts
var __dirname = "/Users/jeon-yunjae/.claude/skills/gstack/browse/src";
...
function resolveNgrokAuthtoken() {
  ...
  if (fs17.existsSync(ngrokEnvPath)) {
    try {
      const envContent = fs17.readFileSync(ngrokEnvPath, "utf-8");
  ...
  for (const conf of ngrokConfigs) {
    try {
      const content = fs17.readFileSync(conf, "utf-8");
...
function readTerminalPort() {
  try {
    ...
    const v = parseInt(fs17.readFileSync(f, "utf-8").trim(), 10);
...
function readTerminalInternalToken() {
  try {
    ...
    const t = fs17.readFileSync(f, "utf-8").trim();
...
`) + `
`;
      fs17.appendFileSync(CONSOLE_LOG_PATH, lines);
...
`) + `
`;
      fs17.appendFileSync(NETWORK_LOG_PATH, lines);
...
`) + `
`;
      fs17.appendFileSync(DIALOG_LOG_PATH, lines);
...
function emergencyCleanup() {
  ...
  isShuttingDown = true;
  try {
    if (fs17.existsSync(config.stateFile)) {
      const raw = fs17.readFileSync(config.stateFile, "utf-8");
  ...
  const makeFetchHandler = (surface) => async (req) => {
    ...
    if (url.pathname === "/welcome") {
      const welcomePath = (() => {
        ...
        if (fs17.existsSync(projectWelcome))
        ...
        if (fs17.existsSync(builtinWelcome))
    ...
    if (url.pathname === "/tunnel/start" && req.method === "POST") {
      ...
      try {
        ...
        const stateContent = JSON.parse(fs17.readFileSync(config.stateFile, "utf-8"));
        stateContent.tunnel = { url: tunnelUrl, domain: domain || null, startedAt: new Date().toISOString() };
        const tmpState = config.stateFile + ".tmp";
        fs17.writeFileSync(tmpState, JSON.stringify(stateContent, null, 2), { mode: 384 });
        fs17.renameSync(tmpState, config.stateFile);
...
`));
  ...
  fs17.writeFileSync(tmpFile, JSON.stringify(state, null, 2), { mode: 384 });
  fs17.renameSync(tmpFile, config.stateFile);
  ...
  try {
    ...
    if (fs17.existsSync(stateDir)) {
      const SEVEN_DAYS = 604800000;
      for (const file of fs17.readdirSync(stateDir)) {
...
start().catch((err) => {
  ...
  try {
    ...
    fs17.mkdirSync(config.stateDir, { recursive: true, mode: 448 });
    fs17.writeFileSync(errorLogPath, `${new Date().toISOString()} ${err.message}
${err.stack || ""}
`, { mode: 384 });
  } catch {}
  process.exit(1);
